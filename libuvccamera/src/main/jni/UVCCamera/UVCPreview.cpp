...
//	
	...
	//pthread_mutex_init(&sy_mutex, NULL);//feiling add
	EXIT();
}
...
UVCPreview::~UVCPreview() {
	...
	//pthread_mutex_destroy(&sy_mutex);//feiling add
	EXIT();
}
...
uvc_frame_t *UVCPreview::get_frame(size_t data_bytes) {
	...
	if UNLIKELY(!frame) {
		...
		//LOGW("get_frame allocate new frame %x", frame);
	}
	return frame;
}
...
void UVCPreview::recycle_frame(uvc_frame_t *frame) {
	...
	if (UNLIKELY(frame)) {
		...
		//LOGW("recycle_frame free frame %x", frame);
	}
}
...
void UVCPreview::clear_pool() {
	...
	pthread_mutex_lock(&pool_mutex);
	{
		...
		LOGW("UVCPreview::clear_pool() n=%d", n);
		for (int i = 0; i < n; i++) {
	...
	pthread_mutex_lock(&capture_mutex);
	{
		...
		if (!env->IsSameObject(mFrameCallbackObj, frame_callback_obj))	{
			...
			if (frame_callback_obj) {
				...
				env->DeleteLocalRef(clazz);//feiling add
				env->ExceptionClear();
			    ...
			    LOGE("UVCPreview : if (frame_callback_obj) = false, frame_callback_obj=%p", frame_callback_obj);
			    env->ExceptionClear();
			}//end
		}
		else {// feiling add
		    //LOGE("UVCPreview : env->DeleteGlobalRef(frame_callback_obj)");
			env->DeleteGlobalRef(frame_callback_obj);
		}//end
		if (frame_callback_obj) {
...
int UVCPreview::startPreview() {
	ENTER();
    LOGD("feiling UVCPreview.cpp startPreview");
...
int UVCPreview::stopPreview() {
	ENTER();
	bool b = isRunning();
	LOGI("UVCPreview::stopPreview() isRunning()=%d", b);
	if (LIKELY(b)) {
		mIsRunning = false;
		mIsCapturing = false;//feiling add
		pthread_cond_signal(&preview_sync);
		pthread_cond_signal(&capture_sync);//feiling rm
		if (pthread_join(capture_thread, NULL) != EXIT_SUCCESS) {//feiling rm
			LOGE("UVCPreview::terminate capture thread: pthread_join failed");
		}
		if (pthread_join(preview_thread, NULL) != EXIT_SUCCESS) {
			LOGE("UVCPreview::terminate preview thread: pthread_join failed");
		}
		clearDisplay();
	}
	clearPreviewFrame();
	...
	//这里了可能内存泄漏，我们调用一次释放内存4
	clear_pool();

	pthread_mutex_lock(&preview_mutex);
	if (mPreviewWindow) {
		//LOGE("UVCPreview : ANativeWindow_release(mPreviewWindow), mPreviewWindow=0x%x", mPreviewWindow);
		ANativeWindow_release(mPreviewWindow);
		mPreviewWindow = NULL;
	}
	pthread_mutex_unlock(&preview_mutex);
	pthread_mutex_lock(&capture_mutex);
	if (mCaptureWindow) {
	    //LOGE("UVCPreview : ANativeWindow_release(mCaptureWindow), mCaptureWindow=0x%x", mCaptureWindow);
		ANativeWindow_release(mCaptureWindow);
		mCaptureWindow = NULL;
	}
	pthread_mutex_unlock(&capture_mutex);
...
void UVCPreview::uvc_preview_frame_callback(uvc_frame_t *frame, void *vptr_args) {
	...
	if UNLIKELY(!preview->isRunning() || !frame || !frame->frame_format || !frame->data || !frame->data_bytes) {
	    if(0)
            ...
            {
                LOGI("feiling uvc_preview_frame_callback recycle_frame 释放内存5");
...
#endif
        //feiling add 这里可能内存泄漏，释放内存6
	    if(0)
            if(frame != NULL && preview != NULL)
            {
                LOGI("feiling uvc_preview_frame_callback recycle_frame 释放内存6");
                preview->recycle_frame(frame);
                frame = NULL;
            }
            //end
...
#endif
            //feiling add 这里可能内存泄漏，释放内存7
            if(0)
                if(frame != NULL && preview != NULL)
                {
                    LOGI("feiling uvc_preview_frame_callback recycle_frame 释放内存7");
                    preview->recycle_frame(frame);
                    frame = NULL;
                }//feiling end
			return;
	...
	{
	    //这里删除frame防止泄露, 释放内存3. 这里如果删除会导致后续do preview中调用的uvc_stop_streaming释放野指针死机。
	    if(0)
            if(frame != NULL )
            {
                preview->recycle_frame(frame);
...
uvc_frame_t *UVCPreview::waitPreviewFrame() {
	...
	pthread_mutex_lock(&preview_mutex);
	{
		if (!previewFrames.size()) {//if (!previewFrames.size() && isRunning()) {//feiling
...
int UVCPreview::prepare_preview(uvc_stream_ctrl_t *ctrl) {
	...
	if (LIKELY(!result)) {
		...
		if (LIKELY(!result)) {
			...
			LOGI("feiling frameSize=(%d,%d)@%s", frameWidth, frameHeight, (!requestMode ? "YUYV" : "MJPEG"));
			pthread_mutex_lock(&preview_mutex);
			if (LIKELY(mPreviewWindow)) {
				ANativeWindow_setBuffersGeometry(mPreviewWindow,
					frameWidth, frameHeight, previewFormat);
...
void UVCPreview::do_preview(uvc_stream_ctrl_t *ctrl) {
	...
	if (LIKELY(!result)) {
		clearPreviewFrame();
		clearCaptureFrame();//feiling add
		pthread_create(&capture_thread, NULL, capture_thread_func, (void *)this);//feiling rm
			...
			for ( ; LIKELY(isRunning()) ; ) {
				...
				if (LIKELY(frame_mjpeg)) {
					...
					if (LIKELY(!result)) {
						...
						addCaptureFrame(frame);//feiling rm
					} else {
						recycle_frame(frame);
					}
				}
...
#if LOCAL_DEBUG
		LOGI("feiling Streaming finished");
#endif
	} else {
		uvc_perror(result, "feiling failed start_streaming");
	}
...
uvc_frame_t *UVCPreview::waitCaptureFrame() {
	...
	pthread_mutex_lock(&capture_mutex);
	{
		if (!captureQueu && isRunning() && isCapturing()) {//feiling if (!captureQueu) {
...
void UVCPreview::do_capture_surface(JNIEnv *env) {
	...
	for (; isRunning() && isCapturing() ;) {
		...
		if (LIKELY(frame)) {
			...
			if LIKELY(isCapturing()) {//if LIKELY(isCapturing() && mCaptureWindow) {//feiling
				if (UNLIKELY(!converted)) {
				    LOGI("do_capture_surface---get_frame");
	...
	if (LIKELY(frame)) {
		...
		if (mFrameCallbackObj) {
			if (mFrameCallbackFunc) {
			    //LOGI("do_capture_callback---get_frame");
			...
			if(!mIsRunning||!mIsCapturing){
			LOGW("camera preview is stop");
                  return;
             }
