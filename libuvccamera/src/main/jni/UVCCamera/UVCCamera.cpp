...
#include "zxczCameraApi-MLA_v2.2.h"
...
int UVCCamera::setScanningMode(int mode) {
	ENTER();
	LOGI("feiling UVCCamera.cpp, setScanningMode :%d", mode);
...
int UVCCamera::getScanningMode() {
	...
	//if LIKELY((mDeviceHandle) && (mCtrlSupports & CTRL_SCANNING)) {
		...
		LOGI("feiling UVCCamera.cpp,getScanningMode r:%d", r);
		if (LIKELY(!r)) {
			r = mode;
		}
	//}
...
/*
int UVCCamera::setScanningMode2(int mode) {
	ENTER();
	 //int r =openMarioUsb(0x2bc5,0x0619);//读取子设备信息
	int ret = UVC_ERROR_IO;
	if (mPUSupports & CTRL_SCANNING) {
		ret = internalSetCtrlValue(mScanningMode, mode , uvc_get_scanning_mode,uvc_set_scanning_mode);
	}
	RETURN(ret, int);
}
*/
int UVCCamera::setScanningMode2(int mode) {
    ...
    if LIKELY(mDeviceHandle) {
        ...
        r = uvc_set_ctrl(mDeviceHandle,0x0a,0x03,buf,0x01);//发送 SET CUR命令 21 01 00 03  00 0a 01 00
    }

    LOGI("feiling UVCCamera.cpp,setScanningMode2 uvc_set_ctrl:%d,mode:%d", r,mode);
	RETURN(0, int);
}

// スキャニングモード設定を取得
int UVCCamera::getScanningMode2() {
	ENTER();
	int r = UVC_ERROR_ACCESS;
	//if LIKELY((mDeviceHandle) && (mCtrlSupports & CTRL_SCANNING)) {
	if LIKELY(mDeviceHandle) {
        ...
        r = uvc_get_ctrl(mDeviceHandle,0x0a,0x03,&mode,0x01,UVC_GET_CUR);//a1 81 00 03  00 0a 01 00
        LOGI("feiling UVCCamera.cpp,getScanningMode2 uvc_get_ctrl:%d,mode:%d", r,mode);
		if (LIKELY(r>=0)) {
			r = mode;
		}
	}
...
uvc_device_handle_t *UVCCamera::getDeviceHandle(){
