...
static jint nativeStartPreview(JNIEnv *env, jobject thiz,
	...
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	if (LIKELY(camera)) {
	    uint64_t support;//feiling add
        camera->getProcSupports(&support);
        camera->getCtrlSupports(&support);//end
...
static jint nativeSetFrameCallback(JNIEnv *env, jobject thiz,
	...
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	if (LIKELY(camera)) {
	    //LOGE("serenegiant_usb_UVCCamera : nativeSetFrameCallback() NULL == jIFrameCallback : %d", NULL == jIFrameCallback);
	    if (NULL == jIFrameCallback) {//feiling add
		    result = camera->setFrameCallback(env, NULL, pixel_format);
		} else {//feiling add
		jobject frame_callback_obj = env->NewGlobalRef(jIFrameCallback);
            //LOGE("serenegiant_usb_UVCCamera : env->NewGlobalRef(jIFrameCallback) = %p, frame_callback_obj = %p", jIFrameCallback, frame_callback_obj);
...
static jint nativeSetScanningMode2(JNIEnv *env, jobject thiz,
	ID_TYPE id_camera, jint scanningMode) {
	...
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	if (LIKELY(camera)) {
		result = camera->setScanningMode2(scanningMode);
	}
	LOGI("feiling seren_usb_UVCCamera.cpp, nativeSetScanningMode2");
	RETURN(result, jint);
}

static jint nativeGetScanningMode2(JNIEnv *env, jobject thiz,
	ID_TYPE id_camera) {
	...
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	if (LIKELY(camera)) {
		result = camera->getScanningMode2();
	}
	RETURN(result, jint);
}
...
jint registerNativeMethods(JNIEnv* env, const char *class_name, JNINativeMethod *methods, int num_methods) {
	...
	env->DeleteLocalRef(clazz);//feiling
	return result;
}
...
static JNINativeMethod methods[] = {
	...
	{ "nativeSetScanningMode2",			"(JI)I", (void *) nativeSetScanningMode2 },
    { "nativeGetScanningMode2",			"(J)I", (void *) nativeGetScanningMode2 },
