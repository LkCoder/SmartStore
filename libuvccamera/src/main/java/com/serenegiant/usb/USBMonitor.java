...
public final class USBMonitor {
	...
	public USBMonitor(final Context context, final OnDeviceConnectListener listener,int mode) {
		if (mode ==1){
			TAG2 = "ir-";
		}else{
			TAG2 ="rgb-";
		}
		if (DEBUG) Log.v(TAG, TAG2+"USBMonitor:Constructor");
		if (listener == null)
			throw new IllegalArgumentException("OnDeviceConnectListener should not null.");
		mWeakContext = new WeakReference<Context>(context);
		mUsbManager = (UsbManager)context.getSystemService(Context.USB_SERVICE);
		mOnDeviceConnectListener = listener;
		mAsyncHandler = HandlerThreadHandler.createHandler(TAG+TAG2);
		destroyed = false;
	 ...
	 */
	public void destroy() {
		if (DEBUG) Log.i(TAG, TAG2+"destroy:");
		unregister();
		if (!destroyed) {
			destroyed = true;
	...
	public synchronized void register() throws IllegalStateException {
		...
		if (mPermissionIntent == null) {
			if (DEBUG) Log.i(TAG, TAG2+"register:");
			final Context context = mWeakContext.get();
			if (context != null) {
				mPermissionIntent = PendingIntent.getBroadcast(context, 0, new Intent(ACTION_USB_PERMISSION), 0);
				final IntentFilter filter = new IntentFilter(ACTION_USB_PERMISSION);
				// ACTION_USB_DEVICE_ATTACHED never comes on some devices so it should not be added here
				filter.addAction(UsbManager.ACTION_USB_DEVICE_ATTACHED);//add debug
...
//			if (DEBUG) Log.i(TAG, "unregister:");
	...
	public synchronized boolean requestPermission(final UsbDevice device) {
		if (DEBUG) Log.v(TAG, TAG2+"requestPermission:device=" + device.getManufacturerName()+",getProductId:"+device.getProductId());
	...
	private final BroadcastReceiver mUsbReceiver = new BroadcastReceiver() {
		...
		public void onReceive(final Context context, final Intent intent) {
			...
			if (ACTION_USB_PERMISSION.equals(action)) {
				...
				final UsbDevice device = intent.getParcelableExtra(UsbManager.EXTRA_DEVICE);
				if (DEBUG) Log.i(TAG, TAG2+"ACTION_USB_DEVICE_ATTACHED:");
//				if (device != null) {
//					if (device.getManufacturerName()!=null && device.getManufacturerName().contains("Mario")){
...
//				}
	...
	private final void processConnect(final UsbDevice device) {
		...
		mAsyncHandler.post(new Runnable() {
			@Override
			public void run() {
				if (DEBUG) Log.v(TAG, TAG2+"feiling processConnect:device=" + device.getManufacturerName());
				UsbControlBlock ctrlBlock;
	...
	private final void processCancel(final UsbDevice device) {
		if (destroyed) return;
		if (DEBUG) Log.v(TAG, TAG2+"processCancel:");
	...
	private final void processAttach(final UsbDevice device) {
		if (destroyed) return;
		if (DEBUG) Log.v(TAG, TAG2+"processAttach:");
		if (mOnDeviceConnectListener != null) {
			mAsyncHandler.post(new Runnable() {
				@Override
				public void run() {
	...
	private final void processDettach(final UsbDevice device) {
		if (destroyed) return;
		if (DEBUG) Log.v(TAG, TAG2+"processDettach:");
		if (mOnDeviceConnectListener != null) {
			mAsyncHandler.post(new Runnable() {
				@Override
				public void run() {
...
//		if (DEBUG) Log.v(TAG, "getDeviceKeyName:" + sb.toString());
	...
	public static final class UsbControlBlock implements Cloneable {
		...
		private UsbControlBlock(final USBMonitor monitor, final UsbDevice device) {
			...
			if (mWeakMonitor.get()!=null) Log.i(TAG, "UsbControlBlock:constructor "+mWeakMonitor.get().TAG2);
...
//			}
		...
		public synchronized void close() {
			...
			if (mConnection != null) {
				...
				if (monitor != null) {
					if (DEBUG) Log.i(TAG, "UsbControlBlock close:"+monitor.TAG2);
