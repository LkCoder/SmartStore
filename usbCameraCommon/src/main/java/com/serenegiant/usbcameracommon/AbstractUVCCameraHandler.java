...
abstract class AbstractUVCCameraHandler extends Handler {
    ...
    private static final int MSG_ShotPicture = 11;
    private static final int MSG_PREVIEW_START_A = 12;
    private static final int MSG_PREVIEW_STOP_A = 13;
    private static final int MSG_GET_CAMERA_ID = 15;
    ...
    protected boolean isCameraThread() {
        final CameraThread thread = mWeakThread.get();
        if (DEBUG)
            Log.v(TAG, "feiling isCameraThread thread.getId=" + thread.getId() + ",currentThread.getId=" + Thread.currentThread().getId());
    ...
    public void close() {
        ...
        if (DEBUG) Log.v(TAG, "feiling close:finished");
    }
    ...
    public long getCameraId() {
        if (DEBUG) Log.v(TAG, "getCameraId");
//        checkReleased();
//        sendEmptyMessage(MSG_GET_CAMERA_ID);
        checkReleased();
        final CameraThread thread = mWeakThread.get();
        final UVCCamera camera = thread != null ? thread.mUVCCamera : null;
        if (camera != null) {
            Log.i(TAG, "getCameraId cameraId: " + camera.getCameraNativeId());
            return camera.getCameraNativeId();
        }
        Log.i(TAG, "getCameraId camera == null");
        return -1;
    }

    protected void startPreview(final Object surface) {
        if (DEBUG) Log.v(TAG, "feiling startPreview:");
        checkReleased();
    ...
    public void startPreviewA(final Object surface) {
        if (DEBUG) Log.v(TAG, "feiling startPreviewA:");
        checkReleased();
        if (!((surface instanceof SurfaceHolder) || (surface instanceof Surface) || (surface instanceof SurfaceTexture))) {
            throw new IllegalArgumentException("surface should be one of SurfaceHolder, Surface or SurfaceTexture");
        }
        sendMessage(obtainMessage(MSG_PREVIEW_START_A, surface));
    }

    public void stopPreview() {
        if (DEBUG) Log.v(TAG, "feiling stopPreview:");
        ...
        if (isPreviewing()) {
            final CameraThread thread = mWeakThread.get();
            if (thread == null) {
                if (DEBUG) Log.v(TAG, "feiling stopPreview:thread == null");
                return;
            }
    ...
    public void stopPreviewA() {
        if (DEBUG) Log.v(TAG, "feiling stopPreviewA");
        removeMessages(MSG_PREVIEW_START_A);
        if (isPreviewing()) {
            final CameraThread thread = mWeakThread.get();
            if (thread == null) {
                if (DEBUG) Log.v(TAG, "feiling stopPreviewA:thread == null");
                return;
            }
            synchronized (thread.mSync) {
                sendEmptyMessage(MSG_PREVIEW_STOP_A);
                if (!isCameraThread()) {
                    try {//feiling rm debug
                        thread.mSync.wait();
                    } catch (final InterruptedException e) {
        ...
        if (DEBUG) Log.v(TAG, "stopPreviewA:finished");
    }


    public void setShotPicture(IFrameCallback icb) {
        checkReleased();
        sendMessage(obtainMessage(MSG_ShotPicture, icb));
    ...
    public int setValue(final int flag, final int value) {
        ...
        if (camera != null) {
            if (flag == UVCCamera.PU_BRIGHTNESS) {
                ...
                return camera.getContrast();
            } else if (flag == UVCCamera.CTRL_SCANNING) {
                camera.setFaceOrScan(value);
                return 1;
            } else if (flag == UVCCamera.CTRL_AE) {
                camera.setAutoWhiteBlance(true);
                return 1;
            } else if (flag == 129) {
                camera.setFaceOrScan2(value);
                return 1;
            }
        }
        throw new IllegalStateException();
    ...
    public List<Size> getSurportSizeLists() {
        checkReleased();
        final CameraThread thread = mWeakThread.get();
        final UVCCamera camera = thread != null ? thread.mUVCCamera : null;
        if (camera != null) {
            Log.i(TAG, "getSurportSizeLists camera !=null");
    ...
    public void handleMessage(final Message msg) {
        ...
        switch (msg.what) {
            ...
            case MSG_CLOSE:
                thread.handleClose();
                break;
            case MSG_GET_CAMERA_ID:
                if (DEBUG) Log.v(TAG, "handle MSG_GET_CAMERA_ID");
                mCameraId = thread.handleGetCameraId();
                if (DEBUG) Log.v(TAG, "handle MSG_GET_CAMERA_ID, mCameraId: " + mCameraId);
                break;
            ...
            case MSG_PREVIEW_STOP:
                ...
                break;
            case MSG_PREVIEW_START_A:
                thread.handleStartPreviewA(msg.obj);
                break;
            case MSG_PREVIEW_STOP_A:
                thread.handleStopPreviewA();
                break;
            ...
            case MSG_RELEASE:
                ...
                break;
            case MSG_ShotPicture:
                if (((IFrameCallback) msg.obj) != null) {
                    thread.setIFrameCallback((IFrameCallback) msg.obj);
                }
    ...
    static final class CameraThread extends Thread {
        ...
        public long handleGetCameraId() {
            if (DEBUG) Log.v(TAG_THREAD, "handleGetCameraId");
            if (mUVCCamera == null) {
                return -1;
            }
            return mUVCCamera.getCameraNativeId();
        }
        ...
        public void handleStopPreview() {
            if (DEBUG) Log.v(TAG_THREAD, "handleStopPreview: mIsPreviewing=" + mIsPreviewing);
            if (mIsPreviewing) {
                if (mUVCCamera != null) {
        ...
        public void handleStartPreviewA(final Object surface) {
            if (DEBUG) Log.v(TAG_THREAD, "handleStartPreviewA: mIsPreviewing=" + mIsPreviewing);
            if ((mUVCCamera == null) || mIsPreviewing) {
                if (DEBUG)
                    Log.v(TAG_THREAD, "handleStartPreviewA: mUVCCamera == null) or mIsPreviewing==true");
                return;
            }
...
//			mUVCCamera.updateCameraParams();
            ...
            Log.d("hxw", "start preview time: " + (end - start));
        }

        public void handleStopPreviewA() {
            if (DEBUG) Log.v(TAG_THREAD, "handleStopPreviewA: mIsPreviewing=" + mIsPreviewing);
            if (mIsPreviewing) {
                long start = System.currentTimeMillis();
                if (mUVCCamera != null) {
                    mUVCCamera.stopPreview();
                ...
                long end = System.currentTimeMillis();
                Log.d("hxw", "stop preview time: " + (end - start));
            }
            if (DEBUG) Log.v(TAG_THREAD, "handleStopPreviewA:finished");
        }
        ...
        public void setIFrameCallback(IFrameCallback icb) {
            if (mUVCCamera != null) {
                mUVCCamera.setFrameCallback(icb, UVCCamera.PIXEL_FORMAT_NV21);
            }
        }
        ...
        private final IFrameCallback mIFrameCallback = new IFrameCallback() {
            ...
            public void onFrame(final ByteBuffer frame) {
                ...
                Log.v(TAG_THREAD, "feiling mIFrameCallback2:");
            }
        };
